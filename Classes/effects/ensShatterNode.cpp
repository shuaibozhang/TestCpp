...
CshatterSprite* CshatterSprite::create(const std::string& filename)
{
	auto pRef = new CshatterSprite();
	if (pRef && pRef->initWithFile(filename))
	{
		pRef->autorelease();
		return pRef;
	}
...
bool CshatterSprite::initWithFile(const string&texFileName){
	...
	void CshatterSprite::updateShatterAction(float time,float dt,float growSpeedOfTargetR){
		...
		float initalTargetR=ccpLength(Vec2(contentSize.width,contentSize.height))/2;//radius of srounding circle
		int nRow=(int)m_grid.size();
		...
		for(int i=0;i<nRow;i++){
			for(int j=0;j<nCol;j++){
				...
				Vec2 movVec=ccpMult(dir,movLen);
				frag->setPosition(fragPos+movVec);
				//update opacity
				float opacity=MAX(0,255-255*disToCenter/(initalTargetR+ 200.f));
...
void CshatterAction::startWithTarget(Node *pTarget){
    //reset
